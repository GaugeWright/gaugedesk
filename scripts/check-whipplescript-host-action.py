...
def check_pin(root=ROOT):
    ...
    require(pin.get("contract_path") == "spec/host-action-contract-v4.json", "unexpected contract path")
    require(pin.get("contract_revision") == "whipplescript-host-action/v4.0.0", "unsupported contract revision")
...
def check_resolved(pin, expected_source, consumer_root=ROOT):
    ...
    # Each bundle pins its predecessors, so the whole chain is verified with the
    # owner's validators; legacy checks alone cannot qualify the v4 pin.
    for validator in ("check-host-action-contract.py", "check-host-action-contract-v2.py",
                      "check-host-action-contract-v3.py", "check-host-action-contract-v4.py"):
