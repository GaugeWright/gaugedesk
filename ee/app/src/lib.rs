//! GaugeDesk Administration capability crate (`gaugedesk-ee`, ADR 0121).
//!
//! The enterprise admin/SSO/SCIM control-plane surface over the shared app
//! substrate (`gaugedesk-app`): org administration + the ENTSEC-1 data-route
...
//! projection (`gaugedesk_app::org`), the audit trail (`gaugedesk_app::audit`),
//! the `IdentityProvider` seam (`gaugedesk_app::identity`), and the
//! `Workbench` authorization/actor helpers (`gaugedesk_app::workbench_auth`)
