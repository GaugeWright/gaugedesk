...
export function defaultOption(
  ...
  const m = catalog.find(
    (c) => c.provider === resolvedDefault?.provider && c.id === id,
  );
  return { ...DEFAULT_OPTION, label: `${m?.name ?? id} (default)` };
}

/** The display name for a model an organization connection serves.
 *
 *  An approved model need not appear in the shipped catalog at all — the
 *  organization approves ids on its own connection — so an unknown id is
 *  reported verbatim rather than hidden behind a friendlier guess. */
export function servedModelLabel(
  ...
  return (
    catalog.find((c) => c.provider === provider && c.id === model)?.name ??
    model
  );
}
...
export function modelOptions(
  ...
  const visible = all.filter((m) =>
    enabled !== null ? enabled.has(modelKey(m)) : isDefaultVisible(m),
  );

  if (
    pinned?.id &&
    !visible.some((m) => m.id === pinned.id && m.provider === pinned.provider)
  ) {
    const found = all.find(
      (m) => m.id === pinned.id && m.provider === pinned.provider,
    );
...
export function parseEndpointModels(
  raw: string | null | undefined,
): DeclaredModels {
  if (!raw) return {};
  try {
    ...
    if (parsed && typeof parsed === "object") {
      ...
      for (const [provider, ids] of Object.entries(
...
export function declaredModelsFor(
...
export function defaultVisibleKeys(
  ...
  return new Set(
    pickableModels(linkedAccountProviders, catalog)
...
export function thinkingLevelsFor(
  ...
  const m = pickableModels(linkedAccountProviders, catalog).find(
    (x) => x.id === modelId && x.provider === provider,
  );
...
export function modelAcceptsImages(
  ...
  const m = catalog.find(
    (c) => c.id === pinned.id && c.provider === pinned.provider,
  );
